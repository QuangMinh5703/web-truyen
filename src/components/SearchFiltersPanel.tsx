...
export default function SearchFiltersPanel({
  ...
  return (
    <div className={`bg-gray-900/50 backdrop-blur-md rounded-2xl border border-white/10 shadow-2xl p-4 ${className}`}>
      {/* Header */}
      ...
      <div className={`space-y-5 ${isExpanded ? 'block' : 'hidden lg:block'}`}>
        ...
        {/* Status Filter */}
        <div>
          <h4 className="text-[10px] font-black uppercase tracking-[0.2em] mb-3 text-gray-500">Trạng thái</h4>
          <div className="grid grid-cols-2 gap-3">
            <button
              onClick={() => handleStatusToggle('ongoing')}
              className={`flex flex-col items-center justify-center p-3 rounded-2xl border transition-all ${filters.status?.includes('ongoing')
            ...
            <button
              ...
              className={`flex flex-col items-center justify-center p-3 rounded-2xl border transition-all ${filters.status?.includes('completed')
                ? 'bg-lime-400 text-black border-lime-400 shadow-lg shadow-lime-400/20'
                : 'bg-white/5 border-white/5 text-gray-400 hover:border-white/10'
                }`}
            >
        ...
        {/* Author Filter */}
        <div>
          <h4 className="text-[10px] font-black uppercase tracking-[0.2em] mb-3 text-gray-500">Tác giả</h4>
          <input
            type="text"
            value={authorInput}
            onChange={(e) => setAuthorInput(e.target.value)}
            placeholder="Tìm theo tác giả..."
            className="w-full px-4 py-2 bg-white/5 border border-white/5 rounded-xl text-xs font-bold text-white placeholder:text-gray-600 focus:outline-none focus:border-lime-400/50 focus:ring-1 focus:ring-lime-400/50 transition-all"
          />
        </div>

        {/* Year Range Filter */}
        <div>
          <h4 className="text-[10px] font-black uppercase tracking-[0.2em] mb-3 text-gray-500">Năm phát hành</h4>
          <div className="flex items-center gap-3">
            <input
              ...
              className="w-full px-4 py-2 bg-white/5 border border-white/5 rounded-xl text-xs font-bold text-center text-white placeholder:text-gray-600 focus:outline-none focus:border-lime-400/50 transition-all"
            />
